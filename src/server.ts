...
import { ApiResponse, AdsPowerProfile } from './types';
...
let adspower: AdsPowerClient | null = null;
try {
  adspower = new AdsPowerClient();
  console.log('✅ AdsPower client loaded successfully');
} catch (e) {
  console.log('⚠️  AdsPower client not available:', (e as Error).message);
}

let sellerResearch: any = null;
let outreachEngine: any = null;
let adspowerProfiles: AdsPowerProfile[] = [];
      ...
      try {
        ...
        if (adspowerStatus.success) {
          ...
          try {
            ...
            console.log('⚠️  Could not load AdsPower profiles:', (profileError as Error).message);
          }
        } else {
          console.log('⚠️  AdsPower not connected - will use Puppeteer directly');
        }
      } catch (adspowerError) {
        console.log('⚠️  AdsPower connection failed:', (adspowerError as Error).message);
...
app.get('/api/adspower/profiles', async (req: Request, res: Response) => {
  try {
    ...
    try {
      ...
      console.log('Could not fetch AdsPower profiles:', (fetchError as Error).message);
      res.json({
        success: true,
        data: [],
        count: 0,
...
app.get('/api/adspower/profiles/:profileId/status', async (req: Request, res: Response) => {
  try {
    const profileId = Array.isArray(req.params.profileId) ? req.params.profileId[0] : req.params.profileId;
    const usage = await db.checkAdsPowerUsage(profileId);
...
app.post('/api/adspower/profiles/:profileId/start', async (req: Request, res: Response) => {
  try {
    if (!adspower) {
      return res.status(503).json({ success: false, error: 'AdsPower not configured' });
    }
    const profileId = Array.isArray(req.params.profileId) ? req.params.profileId[0] : req.params.profileId;
    const result = await adspower.startProfile(profileId, req.body);
...
app.post('/api/adspower/profiles/:profileId/stop', async (req: Request, res: Response) => {
  try {
    if (!adspower) {
      return res.status(503).json({ success: false, error: 'AdsPower not configured' });
    }
    const profileId = Array.isArray(req.params.profileId) ? req.params.profileId[0] : req.params.profileId;
    const result = await adspower.stopProfile(profileId);
    res.json({ success: true, data: result });
  } catch (error) {
    res.status(500).json({ success: false, error: (error as Error).message });
  }
  ...
  try {
    ...
    setImmediate(async () => {
      try {
        ...
        const useProfile: string | null = (adspowerProfileId && adspowerProfiles.length > 0) 
          ? adspowerProfiles.find(p => p.user_id === adspowerProfileId)?.user_id || null 
          : null;
        
        if (useProfile) {
          console.log(`Using AdsPower profile: ${useProfile}`);
        }
    ...
    const useProfileForResponse: string | null = (adspowerProfileId && adspowerProfiles.length > 0) 
      ? adspowerProfiles.find(p => p.user_id === adspowerProfileId)?.user_id || null 
      : null;
    ...
    res.json({
      ...
      keywords: keywords,
      usingAdsPower: !!useProfileForResponse
    });
  } catch (error) {
