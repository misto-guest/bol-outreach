...
/**
 * Enhanced Puppeteer debugging utility
 */
export class PuppeteerDebugger {
   ...
   */
  async launchBrowser(options: puppeteer.LaunchOptions = {}): Promise<Browser> {
   ...
   */
  async connectToBrowser(browserWSEndpoint: string): Promise<Browser> {
      ...
      try {
        ...
        if (element) {
          debugInfo.selectors.contactButtons.push({
            selector,
            text: await this.page.evaluate(el => el.textContent?.trim(), element)
          });
        }
      } catch (error) {
      ...
      try {
        ...
        if (element) {
          debugInfo.selectors.messageFields.push({
            selector,
            placeholder: await this.page.evaluate(el => el.placeholder, element)
          });
        }
      } catch (error) {
        // Ignore errors for optional selectors
